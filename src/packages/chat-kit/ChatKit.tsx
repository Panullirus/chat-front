...
export class ChatKit {
    ...
    async getUserList() {
        return fetch(`http://${this.env.SERVER_URI}:3000/get_users`)
    }

    async verifyMessageRoom(messageRoomData: any) {
        return await axios.post(`http://${this.env.SERVER_URI}:3000/find_message_room`, messageRoomData)
    }

    async createMessageRoom(messageRoomData: any) {
        return await axios.post(`http://${this.env.SERVER_URI}:3000/create_message_room`, messageRoomData)

    }

    async getAllMessage(conversaciones_id: any) {
        return await axios.post(`http://${this.env.SERVER_URI}:3000/get_all_message`, conversaciones_id)
    }


    async sendMessage(message: any) {
        return await axios.post(`http://${this.env.SERVER_URI}:3000/send_message`, message)
    }

    async updateUser(user_data: any){
        return await axios.post(`http://${this.env.SERVER_URI}:3000/update_user`, user_data)
    }
}